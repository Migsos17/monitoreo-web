...
import socket
import sqlite3
import urllib.request  # Librería nativa para enviar datos por la web

# Configuración del archivo de base de datos
DB_NAME = "red_aesa.db"

# 🚀 NUEVA RUTA: Ahora apunta a la URL del archivo PHP que creamos en el Ubuntu
URL_RECEPTOR = "http://192.168.0.194/subir.php"
...
print(f"📡 Iniciando monitoreo relacional: enviando datos vía HTTP a {URL_RECEPTOR}...")
...
while True:
    ...
    if not equipos:
        ...
        # 🚀 GUARDADO POR HTTP POST DIRECTO AL APACHE DE UBUNTU
        try:
            payload = json.dumps(estado_general).encode('utf-8')
            ...
            with urllib.request.urlopen(req, timeout=5) as response:
                respuesta_servidor = response.read().decode('utf-8')
                if respuesta_servidor != "OK":
                    print(f"⚠️ Servidor respondió algo raro: {respuesta_servidor}")
        except Exception as e:
            print(f"❌ Error de red al enviar datos por HTTP: {e}")
